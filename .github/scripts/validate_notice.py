...
def get_first_level_dependencies(sbom: Dict) -> Set[str]:
    ...
    if not first_level_refs and not dependencies:
        print("WARNING: No dependency graph found in SBOM.")
...
def main():
    ...
    if passed:
        print("OK NOTICE file validation PASSED!")
        ...
        print("WARNING NOTICE file validation SKIPPED!")
        print("  SBOM does not contain dependency graph.")
        print("  This is a warning, not a failure.")
        print()
        sys.exit(0)
    else:
        print("FAILED NOTICE file validation FAILED!")
        print()
